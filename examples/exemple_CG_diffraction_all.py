...
mesh = create_mesh_circle_in_square(radius=.7, square_size=2, mesh_size=0.2)
...
MASSE_BORD_CG = assemble_surface(
    mesh, ordre, func, "u", "v", methode="CG", domaine="all"
)
...
U_sol = MAT_EF_CG.solve(dn_F_CG_FOURIER + alpha * F_CG_FOURIER)
...
plot_nodal_vector_CG(U_ex, mesh, ordre, "Solution exacte",secondes=0)

# Représentation de l'erreur
U_err = U_sol - U_ex
print("Norme de l'erreur : ", np.linalg.norm(U_err))
plot_nodal_vector_CG(U_err, mesh, ordre, "Erreur",secondes=0)
